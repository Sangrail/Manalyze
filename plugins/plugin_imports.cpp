...
namespace plugin {
	...
	REQUIREMENT get_requirement(std::string requirement)
	{
		if (requirement == std::string{ "AT_LEAST_ONE" })
			return REQUIREMENT::AT_LEAST_ONE;
		else if (requirement == std::string{ "AT_LEAST_TWO" })
			return REQUIREMENT::AT_LEAST_TWO;
		else if (requirement == std::string{ "AT_LEAST_THREE" })
			return REQUIREMENT::AT_LEAST_THREE;
		else
		{
			std::stringstream ss;
			ss << "Unknown requirement: " << requirement;
			throw std::invalid_argument(ss.str());
		}
	}

	const char* get_threatlevel_name(LEVEL level)
	{
		switch (level) {
		case SAFE:
			return "SAFE";
		...
		case MALICIOUS:
			return "MALICIOUS";
		default:
			throw std::invalid_argument("Unknown threat level");
		}
	}

	LEVEL get_threatlevel(std::string level)
	{
		if (level == (std::string{ "SAFE" }))
			return LEVEL::SAFE;
		else if (level == (std::string{ "NO_OPINION" }))
			return LEVEL::NO_OPINION;
		else if (level == (std::string{ "SUSPICIOUS" }))
			return LEVEL::SUSPICIOUS;
		else if (level == (std::string{ "MALICIOUS" }))
			return LEVEL::MALICIOUS;
		else
		{
			std::stringstream ss;
			ss << "Unknown threat level: " << level;
			throw std::invalid_argument(ss.str());
		}
	}
	/*
	...
	*/
	...
	public:
		...
		pString get_description() const override {
			return boost::make_shared<std::string>("Looks for suspicious ");
		}
		...
		pResult analyze(const mana::PE& pe) override
		{
			...
			try {
				auto anti_debug = _config->at("anti_debug");

				auto vanilla_injection = _config->at("vanilla_injection");
				auto vanilla_injectionThreatLevel = get_threatlevel(_config->at("vanilla_injectionThreatLevel"));
				auto vanilla_injectionRequirement = get_requirement(_config->at("vanilla_injectionRequirement"));
				auto vanilla_injectionDescription = _config->at("vanilla_injectionDescription");

				auto keylogger_api = _config->at("keylogger_api");
				auto raw_socket_api = _config->at("raw_socket_api");
				auto http_api = _config->at("http_api");
				auto registry_api = _config->at("registry_api");
				auto process_creation_api = _config->at("process_creation_api");
				auto process_manipulation_api = _config->at("process_manipulation_api");
				auto service_manipulation_api = _config->at("service_manipulation_api");
				auto privilege_api = _config->at("privilege_api");
				auto dacl_api = _config->at("dacl_api");
				auto dynamic_import = _config->at("dynamic_import");
				auto packer_api = _config->at("packer_api");
				auto temporary_files = _config->at("temporary_files");
				auto hdd_enumeration = _config->at("hdd_enumeration");
				auto driver_enumeration = _config->at("driver_enumeration");
				auto eventlog_deletion = _config->at("eventlog_deletion");
				auto screenshot_api = _config->at("screenshot_api");
				auto audio_api = _config->at("audio_api");
				auto shutdown_functions = _config->at("shutdown_functions");
				auto networking_api = _config->at("networking_api");

				check_functions(pe, registry_api, NO_OPINION, "The program interacts (READS/Writes) with the registry", AT_LEAST_ONE, res);
				...
				check_functions(pe, vanilla_injection, vanilla_injectionThreatLevel, vanilla_injectionDescription, vanilla_injectionRequirement, res);
				...
				PRINT_ERROR << "Import Plugin: Invalid argument: " << ia.what() << ". Check .conf file" << '\n';
			}
